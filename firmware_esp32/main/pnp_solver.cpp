...
// [v2.6.3] Bộ FULL (có cằm P21 + khóe miệng P12/P13) - giầu cấu trúc 3D -> YAW chính xác.
// KHÔNG đổi bộ điểm khi ngáp (đổi bộ gây bước nhảy pose -> hoán đổi trục Y/Z).
// Việc chống lệch khi ngáp do main.cpp xử lý bằng cách GIỮ pose khi MAR>=0.5.
// Camera frame: X right (+X), Y down (+Y), Z toward camera (+Z). Gốc = chóp mũi.
...
bool pnp_solve_head_pose(const point2d_t landmarks_22[22], head_pose_t* out_pose) {
    ...
    float pitch_rad = asinf(-fmaxf(-1.0f, fminf(1.0f, r2[2])));
    float yaw_rad = 0.0f, roll_rad = 0.0f;

    if (cosf(pitch_rad) > 1e-4f) {
        yaw_rad = atan2f(r1[2], r3[2]);
        roll_rad = atan2f(r2[0], r2[1]);
    } else {
    ...
    // [v2.6.1] Sanity gate: loại pose suy biến/lật nghiệm (tránh còi hú giả).
    if (fabsf(out_pose->yaw) > 60.0f || fabsf(out_pose->pitch) > 45.0f || fabsf(out_pose->roll) > 45.0f) {
        out_pose->is_valid = false;
        return false;
    }
